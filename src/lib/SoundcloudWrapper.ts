...
const SoundcloudWrapper = () => {
	...
	const retrieveEndpointChunk = async (endpoint: string, offset = '', limit = 10) => {
		return (await soundcloud.api.getV2(endpoint, {
			offset,
			limit
		})) as SoundcloudRawLibraryChunk;
	};

	const retrieveRawLibrary = async (
		allCrates: Array<SoundRawLibraryEntry> = [],
		nextOffset: string | undefined = undefined
	): Promise<Array<SoundRawLibraryEntry>> => {
	...
	const retrieveLikedTracks = async (
		allLikedTracks: Array<SoundcloudTrackV2> = [],
		nextOffset: string | undefined = undefined
	): Promise<Array<SoundcloudTrackV2>> => {
		const retrievedChunk = await retrieveEndpointChunk(
			`/users/${(await soundcloud.me.getV2()).id}/track_likes`,
			nextOffset
		);
		if (retrievedChunk.collection.length > 0) {
			const remapped = (retrievedChunk.collection as Array<SoundcloudRawTrackEntry>).map(
				(entry) => entry.track
			);
