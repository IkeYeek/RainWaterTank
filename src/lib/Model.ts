...
const syncLibrary = async (
	library: Array<SoundcloudPlaylistV2>,
	userTracks: Array<SoundcloudTrackV2>,
	likedTracks: Array<SoundcloudTrackV2>,
	userMe: SoundcloudUser
) => {
	const db = await connectToDatabase();
	const knownTracksPermas = new Map<
		string,
		mongoose.Document<unknown, object, Track> & Track & { _id: mongoose.Types.ObjectId }
	>();
	...
	for (const soundcloudPlaylistV2 of library) {
		let collection:
			| typeof LikedPlaylistsModel
			| typeof UserPlaylistsModel
			| typeof LikedPlaylistsModel;
	...
	return {
		...
		likedAlbums: likedAlbums.map((likedAlbum) => {
			const ret: Playlist = {
				...
				tracks: likedAlbum.tracks.map(
					(trackId: mongoose.Types.ObjectId) => tracks.find((track) => track._id.equals(trackId))!
				)
			};
			return ret;
		}),
		likedPlaylists: likedPlaylists.map((likedPlaylist) => {
			const ret: Playlist = {
				...
				tracks: likedPlaylist.tracks.map(
					(trackId: mongoose.Types.ObjectId) => tracks.find((track) => track._id.equals(trackId))!
				)
			};
			return ret;
...
export { syncLibrary, getLastSyncedLibraryData, getLastSyncedLibrary, switchSyncStatus };
